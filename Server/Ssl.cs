...
namespace CloudServer
{
    public class Ssl : IDisposable
    {
        ...
        public Ssl(SslContext context, IPAddress address, int port) : this(context, new IPEndPoint(address, port)) { }
        /// <summary>
        /// Initialize SSL server with a given IP address and port number
        /// </summary>
        /// <param name="context">SSL context</param>
        /// <param name="address">IP address</param>
        /// <param name="port">Port number</param>
        public Ssl(SslContext context, string address, int port) : this(context, new IPEndPoint(IPAddress.Parse(address), port)) { }
        /// <summary>
        /// Initialize SSL server with a given DNS endpoint
        /// </summary>
        /// <param name="context">SSL context</param>
        /// <param name="endpoint">DNS endpoint</param>
        public Ssl(SslContext context, DnsEndPoint endpoint) : this(context, endpoint as EndPoint, endpoint.Host, endpoint.Port) { }
        /// <summary>
        /// Initialize SSL server with a given IP endpoint
        /// </summary>
        /// <param name="context">SSL context</param>
        /// <param name="endpoint">IP endpoint</param>
        public Ssl(SslContext context, IPEndPoint endpoint) : this(context, endpoint as EndPoint, endpoint.Address.ToString(), endpoint.Port) { }
        /// <summary>
        ...
        private Ssl(SslContext context, EndPoint endpoint, string address, int port)
        {
            Id = Guid.NewGuid();
            Address = address;
            Port = port;
        ...
        /// </summary>
        /// <param name="id">SessionInfo Id</param>
        /// <returns>SessionInfo with a given Id or null if the session it not connected</returns>
        ...
        /// </summary>
        /// <param name="session">SessionInfo to register</param>
        ...
        public virtual bool Multicast(byte[] buffer, long offset, long size)
        {
            ...
            // Create a new byte array to hold the data to be multicasted
            byte[] dataToMulticast = new byte[size];
            Array.Copy(buffer, offset, dataToMulticast, 0, size);

            // Call the existing Multicast method that accepts byte[]
            return Multicast(dataToMulticast);
        }
