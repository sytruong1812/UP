...
namespace CloudServer
{
    public class SslSession : IDisposable
    {
        ...
        /// Connect the session
        /// </summary>
        /// <param name="socket">SessionInfo socket</param>
        ...
        {
            // Create a new byte array to hold the data to be sent
            byte[] dataToSend = new byte[size];
            Array.Copy(buffer, offset, dataToSend, 0, size);

            // Call the existing Send method that accepts byte[]
        ...
        public virtual bool SendAsync(byte[] buffer, long offset, long size)
        {
            ...
            // Call the existing SendAsync method
            return SendAsync(dataToSend);
        }

        /// <summary>
        /// Send data to the client (asynchronous)
        /// </summary>
        /// <param name="buffer">Buffer to send as an array of bytes</param>
        /// <returns>'true' if the data was successfully sent, 'false' if the session is not connected</returns>
        public virtual bool SendAsync(byte[] buffer)
        {
            ...
            lock (_sendLock)
            {
                ...
                _sendBufferMain.Append(buffer); // Assuming _sendBufferMain has an Append method that accepts byte[]
        ...
        private void TryReceive()
        {
            ...
            try
            {
                ...
                //TODO: HTTPS loop receive request
                do
                {
                    if (!IsHandshaked)
                    {
        ...
        private void ProcessReceive(IAsyncResult result)
        {
            ...
            try
            {
                ...
                // If zero is returned from a read operation
                if (size > 0)
                {
        ...
        /// <summary>
        /// SessionInfo socket disposed flag
        /// </summary>
