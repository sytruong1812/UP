...
void Usage() {
	...
	std::wcout << L"\t\t/from    : Start date time." << std::endl;
	std::wcout << L"\t\t/to      : End date time." << std::endl;
	std::wcout << L"\tOPTION:" << std::endl;
	std::wcout << L"\t\t/see     : Specify the decryption key a opened database connection (MD5 hash)(File input)." << std::endl;
	std::wcout << L"\t\t/aes     : Specify the encryption key for data encrypted with Aes-256-gcm in a blob column (SHA-256 hash)(File output)." << std::endl;
	std::wcout << L"\t\t/expires : Datetime string to update the \'expires_utc\' column (default = current + 1 year)." << std::endl;
	std::wcout << L"\t\t/limit   : Number of rows backup/import per section (default = 100 rows)." << std::endl;
	std::wcout << L"\t\t/delete  : Choose delete old row table in the output file if exist." << std::endl;
	...
	std::wcout << L"\tEx1: " << tool_name << L" /input C:\\Document\\file.db /output C:\\Document\\To\\import.db /see \"Key123\" /aes \"Key456\" /thread 5 /limit 200 " << std::endl;
	std::wcout << L"\tEx2: " << tool_name << L" /input C:\\Document\\file.db /output C:\\Document\\To\\import.db /expires \"2024-10-21 10:10:00\" /thread 5 /limit 200 " << std::endl;
	std::wcout << L"\tEx3: " << tool_name << L" /input C:\\Document\\file.db /output C:\\Document\\To\\import.db /column \"creation_utc\" /from \"13-10-2024\" /to \"15-10-2024\" " << std::endl;
}
...
void importFile(BackupImport* helper, const std::wstring& path_input, const std::wstring& path_output)
{
	int result = FUNCTION_DONE;
	if ((result = helper->importDatabase(path_input, path_output)) != FUNCTION_DONE) {
...
void WINAPI ThreadProc(thread_info* info)
{
	...
	importFile(info->object, info->path_in, info->path_out);
...
void importFolder(BackupImport* base, int max_thread, const std::wstring& path_input, const std::wstring& path_output)
{
	...
	std::wcout << L"--> Number of file import: " << lFilePath.size() << std::endl;
...
static void CommandLine(int argc, wchar_t* argv[])
{
	...
	for (int i = 1; i < argc; i++)
	{
		...
		case OPTION::KEY_AES:
			if ((i + 1 != argc))
			{
				...
				AesManager::instance()->setEncryptFlag(true);
				AesManager::instance()->setDecryptFlag(false);
		...
		case OPTION::TO:
			...
			break;
		case OPTION::EXPIRES_UTC:
			if ((i + 1 != argc))
			{
				DateTime::instance()->setExpiresUtc(Utils::wstr2str(argv[i + 1]));
				helper->setColumnUtcForUpdate(std::string("expires_utc"));
				i++;
			}
			break;
		case OPTION::DEL_TABLE:
			if ((i != argc))
			{
				helper->setDeleteTable(true);
			}
			break;
		case OPTION::LIMIT:
			if ((i + 1 != argc))
	...
	else
	{
		...
		if (!Utils::path_exists(path_input)) {
			std::wcout << L"\n--> Error: Path input file or folder does not exist!" << std::endl;
		}
		else {
			if (PathIsDirectoryW(path_input.c_str())) {
				std::wcout << "--> Maximum running thread: " << max_thread << std::endl;
				importFolder(helper.get(), max_thread, path_input, path_output);
			}
			else {
				...
				if (Utils::createNestedDir(dir) == false) {
					...
					importFile(helper.get(), path_input, path_output);
				}
			}
		}
