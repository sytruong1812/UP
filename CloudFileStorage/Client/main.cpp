...
void Usage()
{
	...
	std::wcout << "  [10] Upload folder" << std::endl;
	std::wcout << "  [11] Remove file" << std::endl;
	std::wcout << "  [12] Rename file" << std::endl;
	std::wcout << "  [13] Watch folder" << std::endl;
...
void TEST()
{
	...
	FolderInfo sub = folder.GetChildren(L"sub_folder");
	std::wstring full = sub.GetPathToRoot();
