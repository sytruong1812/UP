...
{
	public class StreamingBinaryParser : IStreamingBinaryParser
	{
		/// <summary>
		...
		/// <summary>
		///     Initializes a new instance of the <see cref="StreamingBinaryParser" /> class
		...
		/// </param>
		public StreamingBinaryParser(Stream stream, Encoding encoding, int binaryBufferSize = Constants.DefaultBufferSize, string[] binaryMimeTypes = null, bool ignoreInvalidParts = false)
		...
		///     By default the parser will throw an exception if it encounters an invalid part. set this to true to ignore invalid parts.
		/// </param>
		public StreamingBinaryParser(Stream stream, string boundary = null, Encoding encoding = null, int binaryBufferSize = Constants.DefaultBufferSize, string[] binaryMimeTypes = null, bool ignoreInvalidParts = false)
		{
		...
		private static string DetectBoundary(RebufferableBinaryReader reader)
		{
			...
			if (string.IsNullOrEmpty(line)) 
				throw new MultipartParseException("Unable to determine boundary: either the stream is empty or we reached the end of the stream");
			else if (!line.StartsWith("--")) 
				throw new MultipartParseException("Unable to determine boundary: content does not start with a valid multipart boundary");
