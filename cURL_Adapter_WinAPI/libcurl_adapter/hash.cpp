...
static struct Curl_hash_element* mk_hash_element(
...
void* Curl_hash_add2(struct Curl_hash* h, void* key, size_t key_len, void* p, Curl_hash_elem_dtor dtor)
{
...
int Curl_hash_delete(struct Curl_hash* h, void* key, size_t key_len)
{
	...
	if (h->table)
	{
		...
		for (le = Curl_llist_head(l); le; le = Curl_node_next(le))
		{
			struct Curl_hash_element* he = (Curl_hash_element*)Curl_node_elem(le);
			if (h->comp_func(he->key, he->key_len, key, key_len))
			{
				Curl_node_uremove(le, (void*)h);
				--h->size;
...
void
Curl_hash_clean_with_criterium(struct Curl_hash* h, void* user, int (*comp)(void*, void*))
{
	size_t i;
	if (!h || !h->table)
	{
	...
	for (i = 0; i < h->slots; ++i)
	{
		...
		struct Curl_llist_node* le = Curl_llist_head(list); /* get first list entry */
		while (le)
		{
			struct Curl_hash_element* he = (Curl_hash_element*)Curl_node_elem(le);
...
struct Curl_hash_element* Curl_hash_next_element(struct Curl_hash_iterator* iter)
{
	...
	if (iter->current_element)
	{
		struct Curl_hash_element* he = (Curl_hash_element*)Curl_node_elem(iter->current_element);
		return he;
	}
	return NULL;
}
...
void Curl_hash_offt_init(struct Curl_hash* h, size_t slots, Curl_hash_dtor dtor)
{
