...
namespace Helper 
{
    ...
    std::wstring PathHelper::combinePath(const std::wstring& folderPath, const std::wstring& fileName) {
    
        if (!folderPath.empty() && folderPath.back() != L'\\')
        {
            return folderPath + L'\\' + fileName;
        }
    ...
    void StringHelper::replaceWideSubstring(std::wstring& str, const std::wstring& from, const std::wstring& to)
    {
        size_t pos = 0;
