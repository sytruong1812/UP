...
extern "C" {
    ...
    /* none of these struct members should be referenced directly, use the dedicated functions */
    struct Curl_llist {
    ...
    void Curl_llist_insert_next(struct Curl_llist*, struct Curl_llist_node*, const void*, struct Curl_llist_node* node);

    void Curl_llist_append(struct Curl_llist*, const void*, struct Curl_llist_node* node);
    ...
    /* Curl_llist_tail() returns the last 'struct Curl_llist_node *', which might be NULL */
    ...
    /* Curl_node_prev() returns the previous element in a list from a given Curl_llist_node */
