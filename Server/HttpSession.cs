...
namespace CloudServer
{
    public class HttpSession : TcpSession
    {
        ...
        {
            UserManager.Instance.PrepareSession(Id, STATES.CONNECTED);
            Console.WriteLine($"\nHTTP session with Id {Id} connected!\n");
        } 
        ...
        protected override void OnDisconnected()
        {
            ...
            SendResponseAsync(response.MakeOkResponse($"Session with ID {Id} disconnected!"));
            Console.WriteLine($"\nHTTP session with Id {Id} disconnected!\n");
            UserManager.Instance.UpdateUserSession(Id, STATES.DISCONNECTED);
            UserManager.Instance.RemoveSession(Id);
        }
        ...
        /// <param name="request">HTTP request</param>
        private void OnReceivedRequestHeader(Request request) { }
        ...
        private void OnReceivedRequest(Request request)
        {
            switch (request.Method)
            {
                ...
                case "TRACE":
                    Console.WriteLine(request);
                    SendResponse(response.MakeTraceResponse($"Hello Client"));
                    break;
                default:
                    Console.WriteLine(request);
                ...
                case "files":
                    {
                        if (!string.IsNullOrEmpty(segments[2]))
                        {
                            ...
                            else if (file_action == "compare")
                            {
                                Console.WriteLine("\n=================[ Check file miss ]================");
                                Console.WriteLine(request);
                                await UserOperations.ProcessUploadFileMiss(this, request, response, account);
                            }
                        }
                    }
                    break;
