...
#define cc2cu(x) ((x) == CURLE_TOO_LARGE ? CURLUE_TOO_LARGE : CURLUE_OUT_OF_MEMORY)
/*
...
static CURLUcode Curl_parse_port(struct Curl_URL* u, struct dynbuf* host,
    bool has_scheme)
{
    char* portptr;
 ...
 */
  ...
  */
static int dedotdotify(const char* input, size_t clen, char** outp);
static int dedotdotify(const char* input, size_t clen, char** outp)
{
    char* outptr;
...
static CURLUcode parseurl(const char* url, Curl_URL* u, unsigned int flags)
{
    const char* path;
    size_t pathlen;
    char* query = NULL;
    ...
    if (fragment)
    {
        ...
        if (fraglen > 1)
        {
            ...
            {
                u->fragment = (char*)Curl_memdup0(fragment + 1, fraglen - 1);
                if (!u->fragment)
                {
                    result = CURLUE_OUT_OF_MEMORY;
                    goto fail;
    ...
    query = (char*)memchr(path, '?', pathlen);
    if (query)
    {
        ...
        if (qlen > 1)
        {
            ...
            {
                u->query = (char*)Curl_memdup0(query + 1, qlen - 1);
                if (!u->query)
                {
                    result = CURLUE_OUT_OF_MEMORY;
                    goto fail;
    ...
    else
    {
        if (!u->path)
        {
            u->path = (char*)Curl_memdup0(path, pathlen);
            if (!u->path)
            {
                result = CURLUE_OUT_OF_MEMORY;
                goto fail;
...
CURLU* curl_url(void)
{
    return (Curl_URL*)calloc(1, sizeof(struct Curl_URL));
}
...
{
    struct Curl_URL* u = (Curl_URL*)calloc(1, sizeof(struct Curl_URL));
    if (u)
    {
        DUP(u, in, scheme);
        DUP(u, in, user);
...
CURLUcode curl_url_get(const CURLU* u, CURLUPart what, char** part, unsigned int flags)
{
    ...
    *part = NULL;
    ...
    if (ptr)
    {
        ...
        size_t i = 0;
        *part = (char*)Curl_memdup0(ptr, partlen);
        if (!*part)
            return CURLUE_OUT_OF_MEMORY;
        if (plusdecode)
        {
